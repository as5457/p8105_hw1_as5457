...
#Creating a new data frame called "prob1_df" that contains the variables specified in the prompt.
...
#In the data frame above, rand_samp is a random sample of size 10 from a uniform[0, 5] distribution, vector_logical is indicating whether elements of the sample are greater than 2, vector_char is a 10 word long character variable and vector_factor is a variable containing 10 entries of the a factor variable, male or female. 
...
#Creating data frame with the attributes specified in the prompt
prob2_df = tibble(
...
#In the data frame above, x and y are random samples of size 1000 from a standard normal distribution with mean = 0 and standard deviation = 1, logic_xy is a logical vector indicating whether the x + y > 0, num_xy is a numeric vector created by coercing the above logical vector and fac_xy is a factor vector created by coercing the above logical vector
```
...
The scatterplot produced using the numeric variable led to a range of colors being assigned to the color scheme in the scatterplot. This is because the numeric variable is continuous. The reason why we only see two colors in our plot is because our numeric variable only has two values (0/1).
