...
The size of the dataset is `r nrow(prob2_df)`. The mean of the variable x is `r mean(prob2_df[["x"]])` and median of variable x is `r round(median(prob2_df$x))`. The proportion of cases for which the logical vector is true is `r mean(prob2_df$num_xy)`.
...
```{r}
library(ggplot2)
color_xy = ggplot(prob2_df, aes(x = x, y = y, color= 'red')) + geom_point()
color_xy
